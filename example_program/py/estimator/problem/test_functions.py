...
def test_combinatorial_time(k_range=4):
    ...
    for k in range(1, k_range+1):
        ...
        for mix_type in VARIANT_MIXES:
            combinations[k-1][mix_type] = graphGlobal.get_all_valid_combinations(
                VARIANT_MIXES[mix_type], k=k)
        timers[k-1] = time.perf_counter() - start_time_combinations

    ### GET BEST AND WORST COMBINATIONS ###
    for k in range(0, k_range):
        for mix_type in VARIANT_MIXES:
            best_combinations[k].append(
                get_best_combination(combinations[k][mix_type]))
            worst_combinations[k].append(
                get_worst_combination(combinations[k][mix_type]))

    ### PRINT RESULTS ###
    print(f"<--------------------------------------------------------------------------------->")
    print('INFO FOR MIX A')
    print(
        f'Number of stations in mix A:        {VARIANT_MIXES["mix_a"]["g"] + VARIANT_MIXES["mix_a"]["y"] + VARIANT_MIXES["mix_a"]["b"]}')
    print("<---------------------------------------------------------------------------------->")
    for k in range(k_range-1, -1, -1):
        print(
            f'{"best_combination k=" + str(k+1) + " cost":37} {best_combinations[k][0].cost:<7} {"number of combinations"} {len(combinations[k]["mix_a"]):>7}')
    print("<---------------------------------------------------------------------------------->")
    for k in range(k_range-1, -1, -1):
        print(
            f'{"Best combination k=" + str(k+1) + " nodes":37} {best_combinations[k][0].nodes}')
    print("<---------------------------------------------------------------------------------->")
    for k in range(k_range-1, -1, -1):
        print(
            f'{"Worst combination k=" + str(k+1) + " nodes":37} {worst_combinations[k][0].nodes}')
    print("<---------------------------------------------------------------------------------->")
    for k in range(k_range-1, -1, -1):
        print(
            f'{"Execution time combinations k=" + str(k+1):37} {timers[k]:.2f}s')
...
def profile_combinatorial_time(k=2):
    graphGlobal = LayoutGraph(LAYOUT, WEIGHTS)
    cProfile.run('profile_func()', sort='cumtime')
